...
class SeqDecoder(nn.Module):
    ...
    def forward(self, inp, context, state, parent_index=None):
        """
    ...
    def _run_forward_pass(self, emb, context, state, parent_index=None):
        """
...
class ParserModel(nn.Module):
    ...
    def run_decoder(self, decoder, classifier, q, q_all, q_enc, inp, parent_index=None):
        batch_size = q.size(1)
    ...
    def forward(self, q, q_len, ent, lay, lay_e, lay_len, lay_index, tgt_mask, tgt, lay_parent_index, tgt_parent_index, copy_to_ext, copy_to_tgt):
        ...
        lay_out, lay_attn_scores = self.run_decoder(
            self.lay_decoder, self.lay_classifier, q, q_all, q_enc, dec_in_lay)
        ...
        tgt_inp_emb = self.tgt_embeddings(lay[:-1])
