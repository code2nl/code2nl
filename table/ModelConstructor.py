...
def make_decoder(opt, fields, field_name, embeddings, input_size):
    ...
    if field_name == 'tgt':
        classifier = CopyGenerator(opt.dropout, opt.rnn_size, opt.rnn_size,  fields['copy_to_tgt'].vocab, fields['copy_to_ext'].vocab, opt.copy_prb)
    else:
...
def make_base_model(model_opt, fields, checkpoint=None):
    ...
    tgt_embeddings = make_embeddings(
        fields['lay'].vocab, model_opt.decoder_input_size)
