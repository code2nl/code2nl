...
def id2word(id_list, vocab):
    ...
    # print(id_list)
    for each in id_list:
        token = vocab.itos[each.item()]
        words_list.append(token)
    return words_list
def recover_target_token(lay_skip, pred_list, vocab_tgt, vocab_copy_ext, max_sent_length, token, types, fields):
    r_list = []
    copy_list=[]
    token = id2word(token, fields['copy_to_ext'].vocab)
    # print(token)
    types = id2word(types, fields['ent'].vocab)
    for to, ty in zip (token, types):
        if ty in set(['NUMBER', 'NAME', 'STRING']):
            copy_list.append(to)
    count = 0
    for i in range(max_sent_length):
        if i >= len(lay_skip):
        ...
        if i < len(lay_skip) and lay_skip[i] not in set(['NUMBER', 'NAME', 'STRING', table.IO.SKP_WORD]):
            ...
            if pred_list[i][0] < len(vocab_tgt):
                tk = vocab_tgt.itos[pred_list[i][0]]
            else:
                tk = vocab_copy_ext.itos[pred_list[i][0] - len(vocab_tgt)]
            # if tk in set(copy_list):
            #     tk = "<unk>"
            if tk in set(copy_list):
                count+=1
            elif count < len(copy_list):
                tk = copy_list[count]
        r_list.append(tk)
        if r_list[-1] == table.IO.EOS_WORD:
            r_list = r_list[:-1]
...
class Translator(object):
    ...
    def run_tgt_decoder(self, embeddings, tgt_mask_seq, lay_index_seq, lay_all, decoder, classifier, q, q_all, q_enc, max_dec_len, lay_skip_list, vocab, copy_to_ext, copy_to_tgt, lay_dec=None):
        batch_size = q.size(1)
        decoder.attn.applyMaskBySeqBatch(q)
        dec_list = []
        dec_state = decoder.init_decoder_state(q_all, q_enc)
        ...
        for i in range(min(max_dec_len, lay_dec.size(0))):
            # (1, batch)
            ...
            inp = lay_dec[i,:].unsqueeze(0)          
            topk_cpu = topk(dec_out.data.view(batch_size, -1), 20).cpu()
            dec_list.append(topk_cpu)
        return torch.stack(dec_list, 0)
    def my_tgt_decoder(self, embeddings, tgt_mask_seq, lay_index_seq, lay_all, decoder, classifier, q, q_all, q_enc, max_dec_len, lay_skip_list, vocab, copy_to_ext, copy_to_tgt, lay_list):
        batch_size = q.size(1)
        decoder.attn.applyMaskBySeqBatch(q)
        dec_list = []
        dec_state = decoder.init_decoder_state(q_all, q_enc)
    ...
    def translate(self, batch):
        ...
        # encoding
        q_enc, q_all = self.model.q_encoder(q, lengths=q_len, ent=batch.ent)
        if self.model.opt.seprate_encoder:
            q_tgt_enc, q_tgt_all = self.model.q_tgt_encoder(
                q, lengths=q_len, ent=batch.ent)
        else:
        ...
        tgt_dec = self.run_tgt_decoder(self.model.tgt_embeddings, tgt_mask_seq, lay_index_seq, lay_all, self.model.tgt_decoder,
                                       self.model.tgt_classifier, q, q_tgt_all, q_tgt_enc, self.opt.max_tgt_len, lay_skip_list, self.fields['tgt'].vocab, batch.copy_to_ext, batch.copy_to_tgt, lay_dec)
        # recover target
        tgt_list = []
        for b in range(batch_size):
            tgt = recover_target_token(lay_skip_list[b], [tgt_dec[i, b] for i in range(
                tgt_dec.size(0))], self.fields['tgt'].vocab, self.fields['copy_to_ext'].vocab, lay_dec.size(0), batch.copy_to_ext[:,b], batch.ent[:,b], self.fields)
            tgt_list.append(tgt)

        # (3) recover output
        indices = cpu_vector(batch.indices.data)
        return [ParseResult(idx, lay, tgt)
                for idx, lay, tgt in zip(indices, lay_list, tgt_list)]
